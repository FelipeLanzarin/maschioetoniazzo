...
export function ClientFilterSelect() {
  ...
  // Load selected client name when clientId is in URL
  useEffect(() => {
    const clientId = params.get('clientId')
    const clientName = params.get('clientName')
    if (clientId && clientName) setSelectedName(clientName)
    else if (!clientId) setSelectedName('')
  }, [params])
  ...
  function handleSelect(client: Client) {
    ...
    sp.set('clientId', client._id)
    sp.set('clientName', client.name)
    sp.delete('page')
  ...
  return (
    <>
      <button
        type="button"
        onClick={openModal}
        className="flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-[180px] text-left"
      >
        <span className={`flex-1 truncate ${selectedName ? 'text-slate-900' : 'text-slate-400'}`}>
          {selectedName || 'Filtrar cliente...'}
        </span>
        {selectedName && (
          <span
            role="button"
